...
class BertEncoder(nn.Module):
    def __init__(self, config):
        ...
        for i in range(len(self.layer)):
            if i<len(self.layer)/2:
                self.layer[i]=self.layer[i].cuda(0)
            else:
                self.layer[i]=self.layer[i].cuda(1)
    ...
    def forward(
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            layer_head_mask=layer_head_mask.to(device) if layer_head_mask is not None else None
            ...
            past_key_value=past_key_value.to(device) if past_key_value is not None else None
            #output_attentions=output_attentions.to(device) if output_attentions is not None else None
